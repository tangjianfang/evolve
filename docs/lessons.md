...
| E8 | Substring assertions about a script's source survive behavior-flipping mutations — #11's probes passed 2/6; only executing the behavior catches them | When a check guards a script, run the script against fixture inputs and assert its decision (breaker.sh's 24 fixtures); keep substring checks for wiring that cannot be executed (e.g. a claude prompt string) and mutation-probe both kinds | #11 → #12 (final replay: 15/15 mutants killed, every one behaviorally); #15's first file-wide substring check passed vacuously and had to be line-anchored — the non-executable ceiling held (negation/wrong-destination survive); #18–#19 two-pass inspection caught a real refutation and a fix-introduced orphan | 3 |
| E13 | Delegated visual review returns false positives at a non-trivial rate — held again at ~12% (1 of 8 findings): a "dangling (a–f) label" was refuted by SKILL.md's own trigger standard, the reviewer had only grepped the READMEs | Cross-check every delegated finding against the primary sources before acting on or recording it; strike refuted findings explicitly in the round record so the false positive itself stays documented | 50-iteration review 2026-09-05 (read-only subagent, finding #2 of 8 refuted by skills/evolve/SKILL.md trigger standard (a)–(f)) | 0 |
| E14 | In bash, a second `=~` in the same compound condition clears BASH_REMATCH — the capture groups of the first match vanish (live: `--for 2h` parsed as 0h because a trailing `=~ [0-9]` check wiped the duration groups), and existence-only assertions on script output do not catch it (the dry-run check printed its fields happily while the math was zero) | Capture capture-groups into named variables immediately after the matching `=~` (before any other test — use glob `== *…*` for the companion check); when a check asserts a computed value, pin its magnitude or sign too (a 2h budget must not report ~0 minutes), not just that the output line exists | evolve #36 (live: --for 2h → "~0 min remain"; caught by manual dry-run probe, then pinned by a tightened check) | 0 |
...
| E11 | New mechanisms breed orphans: every state a mechanism writes needs set/terminate/reset ownership in a named step, or the readers depending on it rot silently — three live cases in two days (#10's converged status shipped with no setter instruction, #18's parking patch carried an orphaned reset, #19's fix introduced its own orphan the same round) | When adding any header field, status line, or register that a reader (breaker, next session, driver) depends on, write the ownership into the step list before landing — who sets it, when it takes its terminating value, who resets it on resume — then grep the protocol for the reader and confirm the pairing exists | #18 inspector pass-1 refutation + #19 inspector R1 (fix-introduced orphan caught same-round) | 1 |
...
| E12 | `git checkout -- <file>` reverts ALL uncommitted changes to that file — using it to undo a mutation probe also destroys the round's own uncommitted work in the same file (live: #27's probe revert wiped the rule-5 audit text landed minutes earlier) | Revert probe mutations with a targeted inverse edit, or commit the round's work before probing; never `checkout --` a file carrying uncommitted round work; re-run verify after every revert to confirm what actually remains | evolve #27 (first live trigger of the #23 mid-run capture mechanism) | 0 |
