...
| 资深 UX 质量工程师 | 视觉模型子代理（默认 haiku；用户可指定）· 只读 · 只回问题清单 | 视觉走查（仅 UI 目标且有截图机制）：布局、溢出、对比度、缩放 |
...
2. **视觉 review**：委派视觉模型子代理执行（默认 haiku；用户指定的模型优先）；发现必须复核再动手（实践中约 15% 是幻报）
...
───────── 每轮七步闭环 · i = 1..N · 严格顺序 ─────────
  ...
  1. 选目标        从目标池按 Tier 1→4 取下一项 (指针存 log 头部)
      ↓
  2. 视觉 review   视觉模型子代理 (默认 haiku, 用户可指定),
      │            仅 UI 目标且有截图机制;
      │            发现必须复核 (实践约 15% 是幻报)
      ↓
