...
7. **Record**: append one structured line to `docs/evolve-log.md`, advance the header pointer to the next round, update the header metric counters (findings / fixes / regressions) — count from the round's review notes, and keep the header counters equal to the sum of the round lines (E4) — the `findings` counter sums the round lines' `findings(n)` fields exactly, while `fixes` counts the round's actions that fixed confirmed defects, a **subset of actions** (feature adoptions and releases are not fixes; it has no per-round field of its own, so the suite guards it as fixes ≤ Σ actions) — keep the header's `- epics pending:` line current (ids or none), and own the header `- status:` line: `active` while the run continues, the terminating state (`converged` / `pending-epics`) when it ends, reset to `active` when a later run resumes (the breaker stops on terminating states — a stale one caps the new run at one round):
